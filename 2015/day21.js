...
for(var weaponIndex=0; weaponIndex < shop.weapons.length; weaponIndex++){
    for(var armorIndex=-1; armorIndex < shop.armor.length; armorIndex++){
        ...
        if(playerWins({HP: 100, damage: damage, armor: armor},{HP: boss.HP, damage: boss.damage, armor: boss.armor})){
            if(gold < minimumGold){
                minimumGold = gold;
            }
        }else{
        ...
        for(ringIndex=0; ringIndex<shop.rings.length; ringIndex++){
            ...
            if(playerWins({HP: 100, damage: ringsDamage, armor: ringsArmor},{HP: boss.HP, damage: boss.damage, armor: boss.armor})){
                if(ringsGold < minimumGold){
                    minimumGold = ringsGold;
                }
            }else{
                if(ringsGold > maximumGold){
                    maximumGold = ringsGold;
                }
            }

            for(ringIndex2=0; ringIndex2<shop.rings.length; ringIndex2++){
                ringsDamage = damage + shop.rings[ringIndex].damage
                ringsArmor  = armor  + shop.rings[ringIndex].armor
                ringsGold   = gold   + shop.rings[ringIndex].cost
...
console.log('Minimum Gold for win:', minimumGold);
console.log('Maximum Gold for loose:', maximumGold);