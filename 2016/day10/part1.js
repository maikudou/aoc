...
lineReader.on('line', function (line) {
  const [
    ...
    highToNum,
    botNum
  ] = /^(bot|value) (\d+) (?:gives low to (bot|output) (\d+) and high to (bot|output) (\d+))?(?:goes to bot (\d+))?$/.exec(
    line
  )
  if (what === 'value') {
    if (!map.has(botNum)) {
      ...
      map.get(botNum).chips = map.get(botNum).chips.sort(sorter)
...
lineReader.on('close', function () {
  ...
  while (heap.length) {
    ...
    if (top.instruction.lowTo === 'bot') {
      ...
      map.get(top.instruction.lowToNum).chips = map.get(top.instruction.lowToNum).chips.sort(sorter)
    ...
    if (top.instruction.highTo === 'bot') {
      ...
      map.get(top.instruction.highToNum).chips = map
        .get(top.instruction.highToNum)
        .chips.sort(sorter)
