...
var lineReader = require('readline').createInterface({
    input: require('fs').createReadStream(`${__dirname}/day21.input`)
});
...
function rotate(array, line) {
    const [, rotateType, stepsOrLetter] = /(based on position of letter|left|right) (\S)(?: steps)??/.exec(line);
    if (rotateType === 'left') {
...
function reverse(array, line) {
    ...
    array.splice(reverseStart, reverseEnd - reverseStart + 1, ...array.slice(reverseStart, reverseEnd + 1).reverse());
    return array;
}
...
lineReader.on('line', function(line) {
    ...
    switch (operation) {
        ...
        case 'rotate':
            ...
            var after = start.join('');
            console.assert(before === unRotate(after.split(''), line).join(''), line, before, after, unRotate(after.split(''), line).join(''));
            break;
        case 'reverse':
            var before = start.join('');
