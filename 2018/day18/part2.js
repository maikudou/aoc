...
function step(currentMap) {
  ...
  return nextMap.slice(0).map(row => row.slice(0))
}
...
lineReader.on('close', function () {
  ...
  while (true) {
    ...
    const currentHash = hash(currentMap)
    if (states.has(currentHash)) {
      cycleStart = states.get(currentHash)
      cycleEnd = minutes
      break
    } else {
      states.set(currentHash, minutes)
      steps.set(minutes, currentHash)
    }
  }

  const cycleLength = cycleEnd - cycleStart

  const targetMinutes = minutes + ((1000000000 - cycleStart) % cycleLength)
