...
function reduce(snailNumber) {
  ...
  while (explodable || splittable) {
    if (explodable) {
      // console.log('explodable', explodable)
      ...
      // console.log(render(snailNumber))
      continue
    }
    if (splittable) {
      it++
...
lineReader.on('close', function () {
  nums.forEach(num1 => {
    nums.forEach(num2 => {
      if (num1 !== num2) {
        maxMagnitude = Math.max(maxMagnitude, magnitude(reduce(parse(`[${num1},${num2}]`))))
        maxMagnitude = Math.max(maxMagnitude, magnitude(reduce(parse(`[${num2},${num1}]`))))
      }
    })
  })
