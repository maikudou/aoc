...
lineReader.on('close', function () {
  ...
  map.forEach((_, index) => {
    ...
    if (!regions.has(current)) {
      regions.set(current, { area: 0, perimeter: 0, sides: 0 })
    }
  ...
  map.forEach((_, index) => {
    ...
    const region = regions.get(current)
    if (!union.connected(index, index - width)) {
      if (!union.connected(index, index - width - 1) && union.connected(index, index - 1)) {
        region.sides--
      }
    }
    if (!union.connected(index, index + width)) {
    ...
    if (!union.connected(index, index + 1)) {
      if (!union.connected(index, index - width + 1) && union.connected(index, index - width)) {
        region.sides--
      }
    }
    if (!union.connected(index, index - 1)) {
  ...
  console.log(
    Array.from(regions.values()).reduce((acc, value) => acc + value.area * value.sides, 0)
  )
})
